...
// minimal CTHConstants.Override — only the changed keys, grouped by section.
// The 1.13 ini reader merges "<Name>.Override" per-key on top of the base file (INIReader.cpp:62-69).
function buildOverrideIni(s){
  const d=iniDiff(s).filter(x=>x.file==='CTHConstants.ini');
  if(!d.length) return null;
  const bySec={};
  d.forEach(x=>{ (bySec[x.sec]=bySec[x.sec]||[]).push(x); });
  let out='; NCTH Tuning Lab override for CTHConstants.ini\n'
    +'; Merged per-key on top of the mod\'s CTHConstants.ini at load (the "<Name>.Override" mechanism).\n'
    +'; Only the keys below are changed; everything else stays as the mod ships it.\n';
  for(const sec of ['General','Base CTH','Aiming CTH','Shooting Mechanism']){
    if(!bySec[sec]) continue;
    out+=`\n[${sec}]\n`;
    bySec[sec].forEach(x=>{ out+=`; was ${fmtV(x.cur,x.bool)}\n${x.ini} = ${fmtV(x.prop,x.bool)}\n`; });
  }
  return out;
}
// partial Ja2_Options.ini — merged per-key across the VFS profile stack (UserProf topmost = wins),
// because Ja2.ini lists it under MERGE_INI_FILES. Always carries NCTH = TRUE.
function buildOptionsIni(s){
  const d=iniDiff(s).filter(x=>x.file==='Ja2_Options.INI'||x.sec==='Ja2_Options.INI');
  let out='; NCTH Tuning Lab override for Ja2_Options.ini\n'
    +'; This partial file is MERGED per-key over the mod\'s Ja2_Options.INI (MERGE_INI_FILES mechanism),\n'
    +'; so your full options file is untouched.\n\n'
    +'[Tactical Gameplay Settings]\n'
    +'; enable the New Chance To Hit system\nNCTH = TRUE\n';
  d.forEach(x=>{ out+=`; was ${fmtV(x.cur,x.bool)}\n${x.ini} = ${fmtV(x.prop,x.bool)}\n`; });
  return out;
}
function buildModZip(s){
  const d=iniDiff(s), t=buildTunedIni(s);
  if(!d.length||!t) return null;
  const PROF='Profiles/UserProfile_JA2113';
  const ovr=buildOverrideIni(s), opts=buildOptionsIni(s);
  const readme='NCTH tuning package - generated by the NCTH Tuning Lab\n'
    +'https://tais.github.io/ncth-tuning-lab/\n\n'
    +'=== WHAT THIS IS ===\n'
    +'A NON-DESTRUCTIVE override package using the 1.13 VFS layering. Nothing in your\n'
    +'mod\'s Data-1.13 folder is touched; the game merges these files on top at load.\n\n'
    +'Changes:\n'
    +d.map(x=>`  ${x.ini} = ${fmtV(x.prop,x.bool)}   (was ${fmtV(x.cur,x.bool)})`).join('\n')
    +'\n  NCTH = TRUE   (enables the New Chance To Hit system)\n\n'
    +'=== INSTALL (recommended - profile override) ===\n'
    +'  1. Copy the "Profiles" folder from this zip over your JA2 1.13 install folder\n'
    +'     (next to Ja2.exe). It adds two small files to Profiles/UserProfile_JA2113/:\n'
    +'       CTHConstants.Override   - only the changed CTH keys, merged per-key over the mod\'s file\n'
    +'       Ja2_Options.ini         - partial file: NCTH = TRUE (+ any changed option keys), merged per-key\n'
    +'  2. That\'s it. UNINSTALL = delete those two files.\n\n'
    +'  Notes:\n'
    +'  - The profile folder name matches the standard JA2 1.13 vfs profile (vfs_config.JA2113.ini,\n'
    +'    PROFILE_ROOT = Profiles\\UserProfile_JA2113). If you play UB or a custom vfs profile,\n'
    +'    rename the folder to match your PROFILE_ROOT.\n'
    +'  - Because these merge per-key, the package survives mod updates to CTHConstants.ini.\n\n'
    +'=== ALTERNATIVE (classic full-file replacement) ===\n'
    +'  Also included: Alternative-full-file/Data-1.13/CTHConstants.ini - the complete vanilla\n'
    +'  file with the changes applied in place. Back up your original, then copy it over\n'
    +'  Data-1.13/CTHConstants.ini and set NCTH = TRUE in Data-1.13/Ja2_Options.INI yourself.\n\n'
    +'NCTH was designed and written by Headrock. Analysis & tuning lab: see the site above.\n';
  const files=[
    {name:PROF+'/Ja2_Options.ini', text:opts},
    {name:'README-NCTH-Tuning.txt', text:readme},
    {name:'Alternative-full-file/Data-1.13/CTHConstants.ini', text:t.txt},
  ];
  if(ovr) files.unshift({name:PROF+'/CTHConstants.Override', text:ovr});
...
function exportCard(prefix){ prefix=prefix||'exp';
  return `<div class="card"><h2>Proposed changes <span class="propflag">(vs vanilla)</span></h2>
    ...
    <button id="${prefix}_zip" style="display:none;margin-top:8px" class="chip" title="Non-destructive VFS override package: CTHConstants.Override + partial Ja2_Options.ini (NCTH=TRUE) for Profiles/UserProfile_JA2113, plus a classic full-file alternative and README">Download mod ZIP</button>
